 ...
 * Version: 1.0.0
 */
public class RLinearLayoutManager extends LinearLayoutManager {

    protected static final boolean DEBUG = false;
    protected static final String TAG = "BaseLinearLayoutManager";
    ...
    public RLinearLayoutManager(Context context) {
        this(context, VERTICAL, false);
    }


    public RLinearLayoutManager(Context context, int orientation, boolean reverseLayout) {
    ...
    protected boolean isViewHolderUpdated(RecyclerView.ViewHolder holder) {
        return new ViewHolderWrapper(holder).requireUpdated();
    }

    protected boolean isViewHolderUpdated(View view) {
        return new ViewHolderWrapper(getChildViewHolder(view)).requireUpdated();
    }
...
//            }
    ...
    private void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        //L.e("call: layoutChunk([recycler, state, layoutState, result])-> Position:" + adapterPosition);
        ...
        /***/
        layoutChunk(view, params, recycler, state, layoutState, result);
    }
    ...
    protected void layoutChunkLayoutChild(View view, RecyclerView.LayoutParams params, LayoutState layoutState, LayoutChunkResult result) {
        ...
        if (mShouldReverseLayout) {
            for (int i = childCount - 1; i >= 0; i--) {
                ...
                if (mOrientationHelper.getDecoratedEnd(child) > limit
                    ...
                    // stop here
                    recycleChildren(child, recycler, childCount - 1, i);
                    return;
                }
            }
        } else {
            for (int i = 0; i < childCount; i++) {
                ...
                if (mOrientationHelper.getDecoratedEnd(child) > limit
                        || mOrientationHelper.getTransformedEndWithDecoration(child) > limit) {
                    // stop here
                    recycleChildren(child, recycler, 0, i);
                    return;
                }
            }
        }
    ...
    private void recycleChildren(View child, RecyclerView.Recycler recycler, int startIndex, int endIndex) {
        ...
        if (endIndex > startIndex) {
            for (int i = endIndex - 1; i >= startIndex &&
                    canRecyclerChildren(getAdapterPosition(child)); i--) {
    ...
    protected boolean canRecyclerChildren(int position) {
        return true;
    }
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler,
        ...
        return scrollByInner(dx, recycler, state);
    }
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler,
        ...
        return scrollByInner(dy, recycler, state);
    }
    ...
    /**
     * 先clearView, 然后再AddView, 就可以让View最前显示
     */
    public void addOffFlowView(View view, boolean head) {
        clearView(view);
        addAndHiddenView(view, head);
    }

    //==================================
    // Extends method
    //==================================
    protected void addAndHiddenView(View view, boolean head) {
        int index = head ? 0 : -1;
        addView(view, index);
    ...
    protected void clearView(View view) {
        mChildHelperWrapper.clear(view);
    }
    ...
    static class LayoutState {
        ...
        /**
         * The most recent {@link #scrollByInner(int, RecyclerView.Recycler, RecyclerView.State)}
         * amount.
         */
