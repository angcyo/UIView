...
final class DecodeHandler extends Handler {
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        PlanarYUVLuminanceSource source = mDecodeCallback.getCameraManager()
        ...
        Handler handler = mDecodeCallback.getHandler();
        if (rawResult != null) {
            // Don't log the barcode contents for security.
            long end = System.currentTimeMillis();
            Log.d(TAG, "Found barcode in " + (end - start) + " ms");
            if (handler != null) {
                Message message = Message.obtain(handler, IDecodeCallback.decode_succeeded, rawResult);
